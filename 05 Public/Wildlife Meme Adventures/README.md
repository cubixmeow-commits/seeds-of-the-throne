...
- **Format:** portrait editorial field-poster built around an authentic wildlife photograph
...
Use `$artifact-template-wildlife-meme-adventures` for production. The template controls the recurring hierarchy and visual language; the photograph controls the animal and scene; the episode record controls factual metadata and originality.
...
1. **Intake the photograph.** Keep the full-resolution original outside the GitHub vault. Store only a lightweight preview here, and record the original filename, dimensions, checksum, and known external path in the episode record.
...
6. **Inspect visually.** Reject anatomical drift, duplicated or missing animals, altered species, false environmental details, illegible text, clipped type, branding drift, and compositions that bury the photograph.
...
- Current preview profile: JPEG, longest edge 768 pixels, approximately quality 30.
- Portrait poster previews are normally 512 × 768; landscape source previews are normally 768 × 512.
