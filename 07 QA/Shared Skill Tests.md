...
| `write-seeds-prose` modular suite | Expand the writer into scene/chapter, voice, dialogue, exposition, suspense, research/question, revision, customization, evaluation, anti-AI, and benchmark modules; smoke-test close POV and political-grotesque dialogue | Preserve the active Archive Thriller style and authority boundaries; keep mechanical checks conservative; avoid punishing legitimate dialogue formatting; provide measurable regression hooks | Pass after two refinements. The first narrowed the short-paragraph heuristic to narrative paragraphs so dialogue would not create false positives. The desktop rerun then changed the global short-paragraph ratio into a true consecutive-run check, added typographic-apostrophe support to the repeated-contrast detector, and made the tests discoverable by `unittest`. All eight deterministic cases pass. Full fresh-agent execution of all ten behavioral benchmarks remains pending. |
...
The 2026-08-18 `write-seeds-prose` expansion preserved the same skill folder and frontmatter shape. After desktop reconciliation, the official structural validator passed and the eight deterministic Python regression cases passed under standard `unittest` discovery. The full fresh-agent ten-benchmark pass should still be run when an authorized external-agent QA environment is available.
