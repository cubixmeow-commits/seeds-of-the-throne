...
def main():
    ...
    for path in sorted(WORKSHOP.glob('[0-9][0-9] - *.md')):
        ...
        modules.append({'id':meta['module'],'title':meta['title'],'gate':meta['gate'],'prerequisites':meta.get('prerequisites',''),'path':pathstr,'markdown':text,'html':packet_html})
    if modules:
