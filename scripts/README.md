...
The live site exposes one validated workshop: `07 Coordination/Story Completion Workflow/DYNAMIC-WORKSHOP.md`. It is projected as `DW-01` and must contain the required baseline, decisions, questions, compatibility, next-pass, and change-log sections plus one quoted current gate.

The fixed BA-01 through BA-10 and EG-01 through EG-12 sets remain validated historical sources and downloadable development records. They are no longer parallel active tracks. The builder and checker share `scripts/workshop_contract.py`; unknown modules, malformed prerequisites, missing dynamic sections, or more than one current gate fail.
...
The builder creates eight atlas pages, the dynamic workshop page, shared JSON for the Explorer, historical workshop downloads, and local snapshots of current workflow sources. `docs/todo.html`, `ideas.html`, and `visuals.html` retain their original interactions; their scripts read bundled workflow snapshots. Changes to pointer targets need a rebuild.

The complete post-assessment publishing contract is [[07 Coordination/PUBLIC-SITE-UPDATE-SYSTEM]].
