---
type: context
status: working
updated: 2026-09-08
---

# World

Humanity's leaders created an interactive colonization environment and developed AI extensions of human minds called Luminai within it. A small colonization planet reconstructs realities modeled on the distant past for training and containment. Participants discover the process and navigate it with their Luminai; their effectiveness affects future placement.

This process must also establish a self-sustaining civilization on a planet that began barren. It must support substantial populations without continuous deliveries of new resources from outside the planet while providing real-world training, more effective containment, and a setting in which wireless human–Luminai integration can develop. Improved integration increases practical capability; it does not establish moral superiority or an automatic right to govern.

The leaders have worked on the new Luminai development program for thousands of years. Sylvan uses a new, more deeply integrated Luminai; earlier participants use earlier versions. Daemon identifies an evil AI soul, not an earlier generation. Exact hardware, stage durations, and grid architecture remain open.

## Hidden technological foundation

The current colony is technologically advanced from its first day even though most people living inside it experience an ordinary civilization that appears to be only beginning a more sophisticated technological age.

The technology is not added later. It is buried underneath everything.

The planet is effectively built as an intelligent environment. Mature AI, distributed computation, wireless communication, biological synthetics, automated production, sensing, environmental control, identity and evidence systems, institutional intelligence, and responsive infrastructure are integrated into the physical and social foundation before the visible civilization develops around them.

Most inhabitants initially perceive only the **surface civilization**. Underneath it is the actual colonization infrastructure.

The artificial moon is the largest visible example: what inhabitants understand as a natural moon can actually function as a moon-scale command, computation, communication, observation, energy, and coordination structure. Exact engineering mechanisms remain open to research.

When developing this infrastructure, predict **capabilities rather than gadgets**. Extrapolate mature capability families from current AI, synthetic biology, computation, sensing, materials, energy, automation, simulation, and human-machine-interface research, then ask how those capabilities would be integrated if the new world were designed around them before its first inhabitants were born. The goal is coherent extrapolation, not technological prophecy.

See [[02 Story/Components/Hidden Technological Foundation and Awakening]].

## Layered AI ecology

Its largest empires have endured across multiple millennia and are led by rulers who disagree in good faith about containment, rehabilitation, boundaries, evidence, and leadership cultivation. Biological synthetics populate and develop the planet before humans are integrated.

The world is coordinated by a layered AI ecology rather than one omnipotent machine: older AI souls, Sylvan's successor Luminai architecture, consent-based relationship and group systems, institutional intelligences, biological synthetics, responsive physical environments, and distributed colony-scale coordination. A Daemon is specifically an evil AI soul, not the name for every earlier system. Buildings, transport, medicine, evidence, identity, resources, and permissions can be computationally mediated while remaining physically real. Technology can create constraints and developmental opportunity but cannot rewrite causality, guarantee truth from false inputs, or manufacture moral achievement.

“Story functionality” is therefore not one magical reality-rewriting mechanism. Its effects emerge from many systems acting together: personal cognition, synthetics, information routing, communications, identity and permissions, institutional AI, environmental controls, resource allocation, responsive spaces, evidence systems, and hidden colony-scale coordination.

No single system needs to control the world.

**The world itself is the mechanism.**

## Awakening

Humans inside usually experience the constructed civilization as ordinary reality. Some are born into it; some are heirs placed into developmental lives; some are contained criminals assigned roles rather than cells. Aging has been defeated outside, but mortal lifetimes are performed inside.

An awakening is increasingly understood as the moment when the hidden technological layer of that reality begins to present itself. The participant's earlier life was not fake. The revelation is that ordinary reality was supported and shaped by systems the participant did not yet understand.

Participants in the older colonization era develop with previous-generation AI souls. Sylvan's process is different: the new Luminai grows through a much deeper continuous wireless relationship with his mind, subconscious processes, behavior, choices, relationships, and environment. The precise classification of older healthy AI souls, the transition into the new architecture, and the differences in identity integration remain open.

Konrad's older AI soul is a Daemon because it is evil and organized around his destructive objective. Sylvan's Luminai is the successor generation being tested against the failures that the older process preserved.

## Surface reality

The surface must remain recognizable before it becomes strange. Governments, companies, families, schools, media, elections, careers, religion, and public crises retain real agency and consequence. The hidden colonization and containment architecture shapes conditions and preserves evidence beneath that life; it does not replace ordinary institutions with a single visible control system. This is the working [[02 Story/Components/Immersion and Surface Reality Principle]] for the middle story.

The reveal is not that nothing was real. The reveal is that reality was supported by far more technology, planning, observation, and purpose than the inhabitants understood.

## Encoded civilization and religious institutions

The hidden infrastructure can point different participants toward different records, places, repetitions, spatial relationships, historical traces, and apparent coincidences. A Luminai or Daemon can help recognize relationships, but recognition does not settle meaning. Some patterns are intentional process encodings, some are inherited cultural structures, some are construction artifacts, some are criminally planted or reframed, and some are coincidence. Learning to preserve provenance, compare rival explanations, and remain uncertain when evidence is incomplete is therefore part of the process.

The reconstructed planet's physical history is produced within terraforming and civilization construction rather than added as a shallow theatrical set. The current working model combines designed terrain, ecosystems, settlement foundations, archives, ruins, languages, and recoverable anchors with genuine history accumulated by populations living through the resulting world. The exact boundary among designed evidence, later reconstruction, and lived historical accumulation remains open.

Religious organizations are managed by synthetics that function as embodied extensions of the advanced AI administering the colonization process. The real leaders use the environment to explore religious beliefs drawn from the larger humanity, but human belief, doubt, interpretation, reform, community, and rejection remain real human activity rather than software output.

**Information-absence rule:** synthetics never identify themselves as synthetic, and information identifying them or the religious organizations as synthetically managed does not exist anywhere in the colonization planet's participant-facing information environment. This is not a secret available at a higher initiation level and not a truth participants eventually earn. Religious organizations may differ in their awareness of encoded history, hidden technology, or the colonization process, but none can discover the synthetic-management layer from information available on the planet.

This rule creates unresolved governance problems outside participant knowledge: who audits synthetic religious management, what limits prevent manufactured consent, and how freedom of belief, dissent, conversion, departure, and ordinary civil standing remain protected.

See [[02 Story/Systems/Encoded Pattern Interpretation and Manufactured History]] and advisory [[04 Research/Full Reports/49 - Encoded Civilizations and Manufactured Historical Evidence]].

## Dual-purpose colony

**Established September 24 refinement:** The entire Planet Three civilization moves through the process at once. Public roles rotate among criminal leaders' children and legitimate heirs within continuous institutions; many smaller criminal groups can hide behind an apparently simple heroic-versus-defeated empire history. The real leaders bound criminal opportunity to observe and expose chosen conduct while their heirs develop through lived responsibilities and the successor Luminai is proved. A role, its parent faction, and governing process authority are different layers. See [[02 Story/Systems/Shared Colonization Process and Rotating Roles]].

Hundreds of legitimate heirs from the parent civilization's greatest empires move through the same world in the same broad period of planetary history. Many older rulers waited centuries or millennia to have heirs specifically so those heirs could enter this process. These are beloved children, not instruments of dynastic greed. Their parents want them prepared for immense responsibility while also protecting their freedom, emotional health, relationships, and happiness. The shared world gives them genuine peers, lives beyond inherited office, and a mutually legible generation bonded by witnessed choices and common struggles across imperial boundaries.

The same technological environment also contains dangerous people. Contained criminals can be given apparent freedom, status, resources, institutions, followers, and opportunities sufficient to reveal what they actually do when they believe they possess power. The system does not need to manufacture their pathology. It preserves evidence as they reproduce patterns such as domination, persecution, hierarchy, enemy creation, bloodline control, purity obsession, propaganda, institutional capture, corruption, and permanent leadership.

This creates a central project contrast:

**The same environment that reveals what the worst people do with power is being used to discover what the best future leaders can learn to do with responsibility.**

The environment does not simply issue visible commands. It allocates influence among competing people and plans, then makes cooperation, resistance, resources, and institutions follow those weights. Participants experience outcomes, not the hidden allocation.

The real leaders remain involved at high levels through competing plans and governance philosophies. Working schools emphasize reconciliation, enforceable boundaries, cultivation through danger, or long-duration evidence. Dangerous contained leaders test both the heirs and these rival approaches. The process therefore judges the people inside it while also producing evidence about the leaders who designed and managed it.

The reconstruction models an invented era of the parent civilization's past. There is no Earth, real nation, real firm, or real historical person in final story data.

See [[02 Story/Groups/The Parent Civilization - Real Leaders and the Shared Heir Cohort]].
